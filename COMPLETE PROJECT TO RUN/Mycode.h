...
//Initialize movement parameters
  ...
  for (j = 0; j <=7; ++j)
    { ForceOff[i][j] = 0; state[i][j] = 0; GrnStart[i][j] = 0; TransToPhase[i][j] = 0;
      movements[i][j] = 0; presence[i][j] = 0; on[i][j] = 0; onstart[i][j] = 0; MaxTime[i][j] = 20;
...
//Each direction (EB, SB, WB, NB) [1, 3, 5, 7]
  ...
  for (j = 0; j <=5; ++j)
  for (k = 0; k <=3; ++k)
    fscanf (infile, "%d", &CarVolume[i][j][k]);
...
void out (int v1, int v2, int v3, int v4)
{
  ...
  fprintf (output1, "%d\t%d\t%d\t%d\t%d\t%d\t%d\n",
    event,ncGetIntTime(),v1,v2,v3,v4,state[v2][v3]);
}
...
//  	ncGetIntTime(), inter, current[inter], move0, move1, State0,
...
//This is effectively semi-actuated, coordinated where all the minor movement times are fixed
  if (Mode[inter] == 0)
    ...
    else if (TwoPhase == 0 && inter == 0)
      {
      switch (current[inter])
        {
        case 2: {nextp = 3; break;}
        case 3: {nextp = 1; break;}
        case 1: {nextp = 5; break;}
        case 5: {nextp = 7; break;}
        case 7: {nextp = 6; break;}
        case 6: {nextp = 2; break;}
        }
      }
    else if (TwoPhase == 0 && inter== 1)
      {
      switch (current[inter])
        {
        case 3:{nextp = 0; break;}
        case 0: {nextp = 7; break;}
        case 7: {nextp = 4; break;}
        case 4: {nextp = 3; break;}
        }
      }

    else if (TwoPhase == 0 && inter== 2)
      {
      switch (current[inter])
        {
        case 3: {nextp = 0; break;}
    ...
    else if (TwoPhase == 0 && inter== 3)
      {
      switch (current[inter])
        {
        case 1: {nextp = 3; break;}
        case 3: {nextp = 2; break;}
        case 2: {nextp = 6; break;}
        case 6: {nextp = 7; break;}
        case 7: {nextp = 5; break;}
        case 5: {nextp = 1; break;}
        }
      }

    else if (TwoPhase == 0 && inter== 4)
      {
      switch (current[inter])
        {
        case 7: {nextp = 4; break;}
        case 4: {nextp = 2; break;}
        case 2: {nextp = 3; break;}
        case 3: {nextp = 1; break;}
        case 1: {nextp = 7; break;}
        }
      }

    else if (TwoPhase == 0 && inter== 5)
      {
      switch (current[inter])
        {
        case 5: {nextp = 7; break;}
        case 7: {nextp = 6; break;}
        case 6: {nextp = 1; break;}
        case 1: {nextp = 3; break;}
        case 3: {nextp = 2; break;}
        case 2: {nextp = 5; break;}
        }
      }
...
//  ncGetIntTime(), inter, Mode[inter], current[inter], TermCond[inter]);
  ...
  switch (onoff)
    ...
    case 0: //car leaves detector
      if (presence[inter][bae] == 0)
        {
        CurOn[inter][bae] += (ncGetIntTime() - onstart[inter][bae]);
        on[inter][bae] = 0;
        }
      break;
    case 1: //car enters detector
      if (on[inter][bae] == 0) onstart[inter][bae] = ncGetIntTime();
      on[inter][bae] = 1;
      break;
    case 2: //signal goes green
      // signal indication data
      CurRed[inter][bae] = ncGetIntTime() - RedStart[inter][bae];
      AvgRed[inter][bae] = 0.3 * AvgRed[inter][bae]
        + 0.7 * CurRed[inter][bae];
      GrnStart[inter][bae] = ncGetIntTime();
      fprintf (output3, "%d\t%d\t%d\t%d\t%d", ncGetIntTime(), inter,
        current[inter], bae, state[inter][bae] );
      for (j = 0; j <=6; j++) fprintf(output3, "\t%d", presence[inter][j] );
      fprintf(output3, "\t%d\n", presence[inter][7] );
      // detector data
      if (on[inter][bae] == 1) CurOn[inter][bae] += (ncGetIntTime() - onstart[inter][bae]);
      if (CurRed[inter][bae] > 0)
        AvgRedOn[inter][bae] = 0.3 * AvgRedOn[inter][bae]
          + 0.7 * (CurOn[inter][bae]/CurRed[inter][bae]);
      else
        AvgRedOn[inter][bae] = 0;
      if (on[inter][bae] == 1) onstart[inter][bae] = ncGetIntTime();
    ...
    case 3: //signal goes red
      // signal indication data
      CurGrn[inter][bae] = ncGetIntTime() - GrnStart[inter][bae];
      AvgGrn[inter][bae] = 0.3 * AvgGrn[inter][bae]
        + 0.7 * CurGrn[inter][bae];
      RedStart[inter][bae] = ncGetIntTime();
      fprintf (output3, "%d\t%d\t%d\t%d\t%d", ncGetIntTime(), inter,
        current[inter], bae, state[inter][bae] );
      for (j = 0; j <=6; j++) fprintf(output3, "\t%d", presence[inter][j] );
      fprintf(output3, "\t%d\n", presence[inter][7] );
...
//If mode = 2, then adjust the offset, otherwise leave it at its initial value
  ...
  fprintf (output6, "%d\t%d\t%d\t%f\t%d\t%d\t%d\n", ncGetIntTime(),
  MasterCycle, inter, OffAdj,Offset[inter], EndCycle, TimeToRelease[inter]);
